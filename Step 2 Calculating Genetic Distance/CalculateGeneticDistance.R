...
ref_strain <- paste(ref_path,"/Wuhan_reference_",pr,".csv",sep="")
...
tp$start <- as.Date(tp$start,'%d/%m/%Y')
tp$end <- as.Date(tp$end,'%d/%m/%Y')

ac_mm <- c()
for (i in 1:length(tp[,1])) {
  row <- match(tp$filenames[i],row.names(mm_table))
  mm <- mm_table[row,]
  ac_mm <- rbind(ac_mm,mm)
}
colnames(ac_mm) <- colnames(mm_table)
tp2 <- cbind(tp1,ac_mm)
p <- c("/Users/clr/Library/CloudStorage/OneDrive-TheChineseUniversityofHongKong/A-VE source-(211223)/D-plot")
write.csv(tp2,paste(p,"(220209) - VE and mismatch.csv",sep="/"),row.names = FALSE)
write.csv(mm_table,paste(p,"(220210) - S mismatch.csv",sep="/"))

df <- read.csv("/Users/clr/Library/CloudStorage/OneDrive-TheChineseUniversityofHongKong/A-VE source-(211223)/D-plot/(220216) - VE and mismatch.csv", header = TRUE, fill = TRUE)
mm_table <- read.csv("/Users/clr/Library/CloudStorage/OneDrive-TheChineseUniversityofHongKong/A-VE source-(211223)/D-plot/(220210) - S mismatch.csv", header = TRUE, fill = TRUE,row.names = 1)
mismatch <- mm(file[22],pre_seq1,outpath,type)
mm_table[22,] <- mismatch
row <- match(rownames(mm_table)[22],df$new_filename)
df[row,21:24] <- mismatch

mismatch <- mm(file[47],pre_seq1,outpath,type)
mm_table[47,] <- mismatch
row <- match(rownames(mm_table)[47],df$new_filename)
df[row,21:24] <- mismatch
